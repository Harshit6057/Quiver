...
const Bookmarks = ({ user, bookmarks = [], onToggleBookmark }) => {
    ...
    return (
      <div className="max-w-5xl mx-auto md:px-8 overflow-x-hidden">
        <header className="mb-12">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tighter headline-font text-white">Saved Fragments</h1>
          <p className="text-sm sm:text-base text-slate-400 font-light mt-2">Sign in to access your bookmarked transmissions.</p>
        </header>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto md:px-8 overflow-x-hidden">
      <header className="mb-12">
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tighter headline-font text-white">Saved Fragments</h1>
        <p className="text-sm sm:text-base text-slate-400 font-light mt-2">Access your bookmarked transmissions across the Ethereal network.</p>
      </header>
      ...
      {validBookmarks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 glass-card rounded-3xl border border-dashed border-white/10">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">No bookmarks yet</h3>
          <p className="text-slate-500 max-w-sm text-center mb-8 text-sm sm:text-base">
        ...
        <div className="space-y-6">
          {validBookmarks.map(item => (
            <article key={item.id || item.post_id} className="glass-card rounded-xl p-5 sm:p-6 border border-white/5">
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                <div className="space-y-2">
                  <p className="text-xs text-slate-500 uppercase tracking-widest">{item.post.community?.name || 'Void'}</p>
                  <Link to={`/post/${item.post.id}`} className="text-lg sm:text-2xl font-bold text-white hover:text-secondary transition-colors break-words">
                    {item.post.title}
                  </Link>
                  <p className="text-sm sm:text-base text-slate-400 line-clamp-2">{item.post.content}</p>
